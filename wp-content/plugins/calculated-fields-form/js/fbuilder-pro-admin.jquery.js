		...
		$.fbuilder[ 'duplicateItem' ] = function( index, parentItem ) 
			{
				...
				items[ fieldsIndex[ items[ index ][ 'parent' ] ] ][ 'duplicateItem' ]( items[ index ].name, item['name'] );
				...
				if( typeof item[ 'duplicateItem' ] != 'undefined' )
				{
					...
					i = 0; h = items[index][ 'fields' ].length;
					for( i; i < h; i++ )
					{
						item[ 'fields' ][ i ] = $.fbuilder[ 'duplicateItem' ]( fieldsIndex[items[index][ 'fields' ][ i ]], item[ 'name' ] );
					}	
				}	
				return item[ 'name' ];
			};
